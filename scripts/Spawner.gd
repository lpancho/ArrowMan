...
var current_position = position_type.right
...
func _ready():
	...
	current_position = randi() % position_type.size()
	match current_position:
		0: # TOP
			position.x = randi() % int(get_viewport().size.x) + bound
		...
		2: # RIGHT
			...
			position.y = randi() % (int(get_viewport().size.y)-bound) + bound

func _process(delta):
	
	# spawner movements
	if !is_random_movement:
		if current_position == position_type.right:
		...
		elif current_position == position_type.top || current_position == position_type.bottom:
	...
	if current_spawn_time == spawn_time_range.max:
		...
		if s_count != m_count:
			emit_signal("create_fruit", current_position, position)
